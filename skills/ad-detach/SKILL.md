...
3. If the user wants to re-attach to the same app, point them at `/android-debugger:ad-attach`.

4. If the user asks why detach is safe ("won't this kill my app?", "what does detach actually do?"), read `skills/ad-detach/references/lifecycle-internals.md` and answer from there — don't paraphrase, the file has the precise `vm.dispose` vs. `vm.exit` distinction.
