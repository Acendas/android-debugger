...
Canonical for `/android-debugger:bisect-flaky`. Two responsibilities:
...
Read by `/android-debugger:bisect-flaky` step 0 (preflight) and step 3 (candidate selection). Update when new flake patterns appear in customer reports — cite the report in the commit so future maintainers know why the entry exists.
