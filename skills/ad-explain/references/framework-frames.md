...
Canonical allowlist for collapsing noisy framework frames in `frame_snapshot` output. Read this from `/android-debugger:ad-explain` (collapse rule for the rendered frame chain) and `/android-debugger:ad-walk` (default `step_into` class-exclusion filters).
