---
name: ad-investigate
description: Investigate an Android bug end-to-end.
...
4. **Interactive path** — once you've classified the shape via the reference, dispatch to the matching specialist skill: `/android-debugger:ad-catch` (crash), `/android-debugger:ad-trace` or set a bp + `:explain` (behavior), `/android-debugger:ad-bisect-flaky` (flaky test), `/android-debugger:ad-walk` (onboarding). For Behavior sub-shapes, follow the per-sub-shape routing in the reference.
...
6. After autonomous dispatch returns, ask whether the user wants to:
   ...
   - Drill into a specific frame interactively (suggests `/android-debugger:ad-explain`).
   - Detach (suggests `/android-debugger:ad-detach`).
