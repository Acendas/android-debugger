...
Canonical for X-02. Read this from `/android-debugger:trace` (logpoint placement strategy), `/android-debugger:walk` (narration unit), and `/android-debugger:bisect-flaky` (where flake-trigger logpoints go for reactive code).
...
- `/android-debugger:trace` (step 2.5: check before placing logpoints).
- `/android-debugger:walk` (when `Continuation.resume*` appears in the stack).
- `/android-debugger:bisect-flaky` (when the suspect codepath is reactive).
