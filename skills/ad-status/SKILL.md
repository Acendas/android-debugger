...
model: haiku
allowed-tools: mcp__plugin_android-debugger_android-debugger__connection_status, mcp__plugin_android-debugger_android-debugger__list_threads, mcp__plugin_android-debugger_android-debugger__render_capabilities
---
...
1. Call `mcp__plugin_android-debugger_android-debugger__connection_status`. The response carries `attached`, `state`, `package`, `serial`, `pid`, `jdwp_port`, `since_ms`, plus breakpoint and watch counts.
...
3. If `state == ATTACHED_PAUSED`, optionally call `mcp__plugin_android-debugger_android-debugger__list_threads` and surface the count of suspended threads + the name of the paused thread (the one carrying the user's code, usually `main`). This helps the user remember "right, we're paused at a breakpoint."
