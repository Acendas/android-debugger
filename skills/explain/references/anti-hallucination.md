...
Canonical owner. Read this from `/android-debugger:explain`, `/android-debugger:catch`, `/android-debugger:trace`, `/android-debugger:walk`, `/android-debugger:bisect-flaky`, and the orchestrator agent. The rules apply whenever Claude is reading a JDI `frame_snapshot` or invoking `evaluate` / `eval_method` against a live VM.
